...
export function OgImagePicker({
  ...
  const [selected, setSelected] = useState<{ id: string; url: string } | null>(
    mediaId && initialUrl ? { id: mediaId, url: initialUrl } : null
  )
    ...
    <div className="flex items-center gap-2">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>Choose a social share image</DialogTitle>
            <DialogDescription>
              Shown when this page is shared on social media. Recommended
              1200×630.
            </DialogDescription>
          </DialogHeader>
          <Tabs defaultValue="upload">
            ...
            <TabsContent value="upload">
              <UploadTab
                onSelect={(asset) => {
                  setSelected({ id: asset.id, url: asset.url })
                  setOpen(false)
                }}
              />
            </TabsContent>
...
function UploadTab({ onSelect }: { onSelect: (asset: MediaAssetDTO) => void }) {
